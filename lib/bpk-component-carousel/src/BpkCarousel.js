...
import React, { type Element, type ChildrenArray, type Node } from 'react';
...
import BpkCarouselIndicator from '../../bpk-component-carousel-indicator';
...
type State = {
  currentIndex: number,
  // $FlowFixMe[value-as-type] - This component is not flow typed or has an exported type
...
class BpkCarousel extends React.Component<Props, State> {
  ...
  onLayout: (event: any) => void = (event: any) => {
    this.setState({
  ...
  layoutProvider: any = new LayoutProvider(
    () => 0,
    (type, dimensions) => {
  ...
  // TODO: Fix this any
  renderItem: any = (type: number, item: Element<BpkCarouselItem>) => item;
