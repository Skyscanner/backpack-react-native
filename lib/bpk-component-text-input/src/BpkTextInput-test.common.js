...
const commonTests = () => {
  describe('BpkTextInput', () => {
    describeEachColorScheme(BpkTextInput, WithColorScheme => {
      ...
      it('should mask correctly when given an unusual value to mask (1)', () => {
      ...
      it('should mask correctly when given an unusual value to mask (2)', () => {
        ...
        */
        const testRenderer = TestRenderer.create(
          <WithColorScheme label="Name" value="abd-efgh" mask="AAAA-AAAA" />,
        );

        expect(testRenderer.toJSON()).toMatchSnapshot();
      });

      it('should mask correctly when given an unusual value to mask (3)', () => {
        /* The value here simulates entering '12ab-56cd', then cursoring back
          ...
          */
        const testRenderer = TestRenderer.create(
          <WithColorScheme label="Name" value="12b-56cd" mask="SSSS-SSSS" />,
        );

        expect(testRenderer.toJSON()).toMatchSnapshot();
      });

      it('should not try to sanitise the masked value when the mask contains "*"', () => {
        const testRenderer = TestRenderer.create(
          <WithColorScheme label="Name" value="124-5678" mask="9999*9999" />,
        );
