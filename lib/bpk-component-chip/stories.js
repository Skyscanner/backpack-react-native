...
const outlineStyles = BpkDynamicStyleSheet.create({
  row: {
    backgroundColor: { light: surfaceContrastDay, dark: canvasNight },
...
storiesOf('bpk-component-chip', module)
  ...
  .add('docs:outline', () => (
    <View
      style={[styles.bottomMargin, useBpkDynamicStyleSheet(outlineStyles).row]}
    >
      <View style={styles.row}>
        {COUNTRIES.map((country, index) => (
          <BpkChip
  ...
  .add('docs:dismissible-outline', () => (
    <View style={[styles.row, useBpkDynamicStyleSheet(outlineStyles).row]}>
      {COUNTRIES.map((country, index) => (
        <BpkDismissibleChip
          key={country}
          label={country}
